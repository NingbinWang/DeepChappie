...
INT sys_medium_fs_type_convert_data(SYS_MEDIUM_FS_TYPE_E eFsType,char *strData,UINT16 uSize)
{
    ...
    switch(eFsType)
    {
        ...
        case SYS_MEDIUM_FS_NTFS:
            ...
            break;
        default:
            SYS_COMMON_ERROR("no support type :%d \n",eFsType);
            return iRet;
    }
...
INT sys_medium_mount_exfat(const char *strDevPath,const char *strMountPath)
{
    ...
    if(iRet < 0)
    {
        /* SYS_COMMON_ERROR("sys_posix_cmd_call:%s error \n",strMountCmd); */
        return iRet;
    }
    return iRet;
}
 ...
 * @return     成果返回OK，失败返回ERROR,
 */
INT sys_medium_mount(const CHAR *strDevPath, SYS_MEDIUM_FS_TYPE_E eFsType, const CHAR *strMountPath)
{
    INT iRet = -1;
    CHAR strData[128] = {0};
    ...
    if(iRet < 0)
    {
        SYS_COMMON_ERROR("sys_medium_fs_type_convert_string type :%d error \n", eFsType);
        return iRet;
    }
    ...
    if(iRet < 0)
    {
        SYS_COMMON_ERROR("convert data type :%d error \n", eFsType);
        return iRet;
    }
...
INT sys_medium_umount(const CHAR *strMountPath)
{
    ...
    if(iRet != OK)
    {
        /* SYS_COMMON_ERROR("umount %s failed, %s\n", strMountPath, strerror(errno)); */
        return iRet;
    }
    return iRet;
}
...
INT sys_medium_format(const CHAR *strDevPath, SYS_MEDIUM_FS_TYPE_E eFsType, UINT uUintSize)
{
    ...
    if(iRet < 0)
    {
        SYS_COMMON_ERROR("strSysFormatCmd %s error \n", strSysFormatCmd);
        return iRet;
    }
    sync();
    return iRet;
...
INT sys_medium_access_dev_node(const CHAR* strDevNodePath)
{
    ...
    if(iRet < 0)
    {
        /* SYS_COMMON_ERROR("open %s failed, %s\n", strDevNodePath, strerror(errno)); */
        return iRet;
    }
...
INT sys_medium_check_dev_node(const CHAR *strDevPath)
{
	...
	for(iCnt = 0; iCnt < 2; iCnt++)        /* 当前尝试两次 */
	{
		...
		{
			/* SYS_COMMON_INFO("check dev node:%s successful\n", strDevPath); */
			return 0;
		}
 ...
 */
INT sys_medium_get_info(const CHAR *strMountPath, SYS_MEDIUM_INFO_T *pStMediumInfo)
{
    INT iRet = -1;
   ...
   // SYS_COMMON_INFO("stInfo.f_bsize = %u\n", stInfo.f_bsize ); 
    ...
    //SYS_COMMON_INFO("strMountPath:%s uTotalSize:%d MB uRemainSize:%dMB f_type:0x%x\n",strMountPath,pStMediumInfo->uTotalSize,pStMediumInfo->uRemainSize,(UINT)stInfo.f_type);
    return 0;
}
