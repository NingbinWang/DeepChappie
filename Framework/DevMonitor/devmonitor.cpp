 ...
 */
INT32 devmonitor_init_hotplug_sock(VOID) 
{ 
    INT32 iRet = ERROR; 
    INT32 iSockfd = ERROR;
 ...
 * @return	   无
 */
void devmonitor_netlink_data_handle(DEV_MONITOR_PRIV_DATA_T *pStPrivData, CHAR *strBuf, UINT32 uLength)
{       
    INT32 iRet = -1;
    INT32 iMediumNo = 0;
 ...
 * @return     无
 */
static INT32 devmonitor_check_dev(IDevMonitor *pIDevMonitor,DEV_MONITOR_TYPE_E eType, INT32 iDevIndex)
{
    DEV_MONITOR_PRIV_DATA_T *pStPrivData = NULL;
 ...
 * @return	   成功返回IDevMonitor操作指针  失败返回NULL;
 */
static INT32 devmonitor_init_interface(IDevMonitor *pIDevMonitor)
{   
    INT32 iRet = ERROR;
    if(NULL == pIDevMonitor)
    {
