...
INT sys_socket_bind(INT iSockFd, INT iDomain, const CHAR *strIPAddr, UINT16 uPort)
{
    ...
    //端口复用
	if (ERROR == setsockopt(iSockFd, SOL_SOCKET, SO_REUSEADDR, (CHAR*)&bReuse, sizeof(INT)))
    {
 ...
 */
INT sys_socket_convert_optlevel_and_name(INT iOptLevel,INT iOptName,INT *pConvertOptName,INT *pConvertName)
{
    INT iRet = ERROR;
    INT iAdapteroptlevel = 0;
    INT iAdapterOptname = 0;
...
{   
    INT iRet = ERROR;
    INT iAdapteOptLevel = ERROR;
    INT iAdapteOptName  = ERROR;
...
INT sys_socket_get_sock_opt(INT iSockFd, INT iOptLevel, INT iOptName, VOID *pOptValue, INT *iOptLength)
{
    ...
    if (getsockopt(iSockFd, iOptLevel, iOptName, pOptValue, (UINT *)iOptLength) < 0)
    {
...
INT sys_socket_get_tcp_state(int iSockFd)
{
    ...
    iLen = sizeof(stInfo);
    iRet = sys_socket_get_sock_opt(iSockFd, SYS_IPPROTO_TCP, TCP_INFO, &stInfo, (INT *)&iLen);
    if(iRet < 0)
    {
        return iRet;
    }
