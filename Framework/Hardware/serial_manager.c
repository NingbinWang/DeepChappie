...
*/
INT uart_get_debug_info(ISerial *pISerial, SERIAL_COUNTER_INFO_T *pStUartInfo)
{
    INT iRet = ERROR;
 ...
 * @return	   成功 0 失败 -1
 */
INT uart_open(ISerial *pISerial)
{   
    INT iFd = -1;
 ...
 * @return	   
 */
static INT uart_release(ISerial *pISerial)
{
    INT iRet = ERROR;
    SERIAL_UART_BASE_T *pBase = NULL;
    if(NULL == pISerial)
 ...
 * @return	   成功 0 失败 -1
 */
INT uart_close(ISerial *pISerial)
{
    INT iRet = ERROR;
    SERIAL_UART_PRIV_DATA_T *pStPrivData = NULL;
    if(NULL == pISerial)
 ...
 */
INT uart_write(ISerial *pISerial, const UINT8 *pData,UINT uLength)
{
    INT iFd  = -1;
    INT iRet = -1;
    SERIAL_UART_PRIV_DATA_T *pStPrivData = NULL;
 ...
 */
INT uart_read(ISerial *pISerial, UINT8 *pData, UINT uLength)
{
    INT iFd  = -1;
    INT iRet = -1;
    SERIAL_UART_PRIV_DATA_T *pStPrivData = NULL;
 ...
 * @return	   成功 0 失败 -1
 */
INT uart_set_attribute(ISerial *pISerial, SERIAL_ATTRIBUTE_UART_T *pStInfo)
{
    INT iRet =-1;
    INT iFd = -1;
 ...
 * @return	   
 */
static INT uart_init_priv_data(SERIAL_UART_PRIV_DATA_T *pStPrivData,CHAR* sDevName, INT iIndex)
{
    INT iRet = ERROR;
    if(NULL == pStPrivData)
    {
 ...
 */
static INT uart_init_interface(ISerial *pISerial)
{   
    INT iRet = ERROR;
    if(NULL == pISerial)
    {
