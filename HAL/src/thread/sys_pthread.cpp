...
INT32 (*sys_printf)(const char *format, ...) = printf;
 ...
 * @note
 */
INT32 sys_pthread_priority_get(INT32 *pMinPrio, INT32 *pMaxPrio)
{
 ...
 * @note
 */
static INT32 sys_pthread_set_attr(pthread_attr_t *attr, INT32 iPriority, size_t* stacksize)
{
    INT32 iRetVal;
    //struct sched_param    params;
   // INT32 maxPriority, minPriority;
        ...
        unsigned args, ...)
{
    ...
    if( iRetVal != 0 )
    {
        SYS_COMMON_ERROR("sys pthread set attr error iRetVal:%d \n",iRetVal);
        return iRetVal;
    }
    ...
    if(iRetVal != 0)
    {
       SYS_COMMON_ERROR("sys pthread create error iRetVal:%d \n",iRetVal);
       free((CHAR *)pstFunc);
	   pthread_attr_destroy(&attr);
       return ERROR;
    }
...
INT32 sys_pthread_cancel(TASK_ID stTaskId)
{
    ...
    {
        SYS_COMMON_ERROR("pthread is already unexit\n");
        return 0;
    }
/*Android cancle接口被取消*/
#ifndef ANDROID
