...
INT sys_block_format(const char *dev)
{
    ...
    {
        formatlen = ((UINT)(size - offset) < SYS_BLOCK_TMPLEN) ? (UINT)(size - offset) : SYS_BLOCK_TMPLEN;
...
{
    return (UINT)sys_block_size(dev);
}
...
UINT sys_block_valid_data_len(const CHAR *dev)
{
    ...
    for (offset = 0; offset < size; offset += SYS_BLOCK_TMPLEN)
    {
        readlen = ((UINT)(size - offset) < SYS_BLOCK_TMPLEN) ? (UINT)(size - offset) : SYS_BLOCK_TMPLEN;
...
INT sys_block_copy(const char *src_dev, const char *dst_dev)
{
    ...
    {
        readlen = ((UINT)(src_size - offset) < SYS_BLOCK_TMPLEN) ? (UINT)(src_size - offset) : SYS_BLOCK_TMPLEN;
