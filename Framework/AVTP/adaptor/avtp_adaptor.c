 ...
 */
static INT avtp_socket_address_setup(INT iSock, CHAR *pInterfaceName, UINT8 *pMacAddr, INT iProtocol, struct sockaddr_ll *pSockAddr)
{
    INT iRet = ERROR;
...
INT avtp_talker_socket_create(INT iPriority, CHAR *pInterfaceName, UINT8 *pDestMacAddr, struct sockaddr_ll *pStSockAddr)
{
    ...
    iSock = socket(AF_PACKET, SOCK_DGRAM, (INT)sys_htons(ETH_P_TSN));
    if(iSock < 0)
    {
