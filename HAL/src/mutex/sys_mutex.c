...
static inline VOID sys_timespec_get(INT iWaitMs, struct timespec *tp)
{
    ...
    {
        // SYS_COMMON_ERROR("getTimespec: clock_gettime call fail, error %d(%s)\n", errno, strerror(errno));
...
INT sys_mutex_create(MUTEX_ID *pStMutex, INT iType)
{
    ...
    if (pStMutex == NULL)
    {
        SYS_COMMON_ERROR("Mutex init invalid ptr");
        return ERROR;
    }
    ...
    *pStMutex = (MUTEX_ID)sys_mem_malloc(sizeof(pthread_mutex_t));
    ...
    if (iRet != 0)
    {
        SYS_COMMON_ERROR("Mutex set type error:%s ret:%d-%s \n", strerror(errno), iRet, strerror(iRet));
        return iRet;
    }
    ...
    if (iRet != 0)
    {
        SYS_COMMON_ERROR("Mutex init error:%s ret:%d-%s \n", strerror(errno), iRet, strerror(iRet));
    }
...
INT sys_mutex_lock(MUTEX_ID *pStMutex, INT iWaitMs)
{
    ...
    {
        SYS_COMMON_ERROR("Mutex lock invalid ptr. \n");
        return ERROR;
    }
...
INT sys_mutex_unlock(MUTEX_ID *pStMutex)
{
    if (pStMutex == NULL || (pthread_mutex_t *)*pStMutex == NULL)
    {
        SYS_COMMON_ERROR("Mutex unlock invalid ptr. \n");
        return ERROR;
    }
...
INT sys_mutex_destroy(MUTEX_ID *pStMutex)
{
    ...
    if (pStMutex == NULL || (pthread_mutex_t *)*pStMutex == NULL)
    {
        SYS_COMMON_ERROR("Mutex destroy invalid ptr. \n");
        return ERROR;
    }

    if ((iRet = pthread_mutex_destroy((pthread_mutex_t *)*pStMutex)) != 0)
    {
        SYS_COMMON_ERROR("Mutex destroy error:%s ret:%d-%s \n", strerror(errno), iRet, strerror(iRet));
        return iRet;
    }
